...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        with init_context(), warnings.catch_warnings():
            ...
            print(f"Model vocab size: {actor_module.config.vocab_size}")  
            # Option A: Resize tokenizer (safer)
            if self.tokenizer.vocab_size < actor_module.config.vocab_size:
                num_added = actor_module.config.vocab_size - self.tokenizer.vocab_size
                new_tokens = [f"<pad_{i}>" for i in range(num_added)]
                self.tokenizer.add_tokens(new_tokens)
                # self.tokenizer.save_pretrained(local_path)
                print(f"✓ Added {num_added} padding tokens to tokenizer")
