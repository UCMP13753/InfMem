        ...
        for i, node in enumerate(nodes):
            ...
            try:
                ...
                actors.append(RewardActor.options(
                        ...
                        # https://docs.ray.io/en/releases-2.10.0/ray-core/scheduling/index.html
...
class ThreadRewardManager:
    ...
    def __call__(self, data: DataProto, given_score=None, return_dict: bool = False):
        ...
        sequences_strs = []
        # try:
        #     steps_num = data.batch["final_steps"]
        #     steps_norm = steps_num / steps_num.max()
        # except:
        #     steps_norm=None
        # steps_norm=None
        ...
        args = [(i, 
                 dict(data_source=data[i].non_tensor_batch['data_source'], 
                    ...
                    #   steps_num=steps_norm[i].item() if steps_norm is not None else None, 
                      ))
                 for i in range(len(data))]
        ...
        if given_score is not None:
            # Ensure tensor form & length
            if not isinstance(given_score, torch.Tensor):
                given_score = torch.as_tensor(given_score, dtype=torch.float32)
            assert len(given_score) == len(data), (
                f"given_score length {len(given_score)} does not match data length {len(data)}"
            )
                    
            # (i -> result) to be robust to ordering
            comp_map = {i: r for (i, r) in results}

            # Mixing coefficient = mean(given_score), clamped to [0,1]
            # alpha = float(torch.clamp(given_score.mean(), 0.0, 1.0))
            alpha = float(given_score.mean())
            logger.info(f"{alpha=}s")
            # if alpha < 0.5:
            #     threshold = 0
            # else:
            #     threshold = alpha
            ...
            # else:
            #     threshold = 0
            # Mix per item: (1 - alpha) * given + alpha * compute
            comp_sum, comp_cnt = 0.0, 0
            results = []
            for i in range(len(data)):
                r = comp_map[i]
                if isinstance(r, dict):
                    comp_score = float(r.pop("score"))
                    # keep any extra info from the computed branch
                    for k, v in r.items():
                ...
                # mixed = (1.0 - threshold) * float(given_score[i]) + threshold * comp_score
                mixed = float(given_score[i]) + 0.6 * comp_score
                
                # mixed = comp_score
                results.append((i, mixed))
                
                      
            avg_comp  = float(comp_sum / comp_cnt) if comp_cnt else float("nan") 
            logger.info(f"{avg_comp=}s")
            reward_extra_info["avg_intermediate_reward"].append(alpha)
            reward_extra_info["avg_groundtruth_reward"].append(avg_comp)
        # Fill reward tensor with results
        for (i, result), valid_response_length in zip(results, valid_response_lengths):
            score: float
            if isinstance(result, dict):
                score = result.pop("score") # for GRPO, do not want duplicate score/reward
