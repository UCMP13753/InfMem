...
def _default_compute_score(data_source, solution_str, ground_truth, extra_info=None, **kwargs):
    ...
    elif 'ruler' in data_source:
        from . import ruler
        if 'qa' in  data_source:
            res = ruler.compute_score_qa(solution_str, ground_truth)
        else:
            res = ruler.compute_score_ruler(solution_str, ground_truth)
    else:
