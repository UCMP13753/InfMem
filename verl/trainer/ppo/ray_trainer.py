    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if not self.config.recurrent.enable:
                ...
                output_gen_batch, final_mask, sample_index, final_rewards = self.generation_manager.run_llm_loop(test_gen_batch, {})
            ...
            #### overwrite the previous reward
            # if final_rewards.dim() == 1:
            #     # Expand to token dim = reward_tensor expected T
            #     T = reward_tensor.shape[1] if "reward_tensor" in locals() else test_batch.batch["input_ids"].shape[1]
            #     reward_tensor = final_rewards.unsqueeze(1).repeat(1, T)
            # else:
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                # with _timer('step', timing_raw):
                            ...
                            gen_batch_output, final_mask, sample_index, final_rewards = self.generation_manager.run_llm_loop(gen_batch, timing_raw)
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if not self.config.recurrent.enable:
                            ...
                            reward_tensor, reward_extra_infos_dict = compute_reward(reward_batch, self.reward_fn, given_score=final_rewards)
                            def to_scalar(x):
                                if x is None: return None
                                if isinstance(x, (list, tuple)):
                                    return None if len(x)==0 else float(x[-1])  # 或改成 sum(x)/len(x)
                            ...
                            avg_inter = to_scalar(reward_extra_infos_dict.get("avg_intermediate_reward"))
                            avg_gt = to_scalar(reward_extra_infos_dict.get("avg_groundtruth_reward"))

                            if avg_inter is not None:
                                metrics["critic/rewards/avg_intermediate_reward"] = avg_inter
                            if avg_gt is not None:
                                metrics["critic/rewards/avg_groundtruth_reward"] = avg_gt
                            # metrics.update(
                            #     {
                            #         "critic/rewards/avg_intermediate_reward": reward_extra_infos_dict.avg_intermediate_reward,
                            #         "critic/rewards/avg_groundtruth_reward": reward_extra_infos_dict.avg_groundtruth_reward,
                            #     }
                            # )
                            # print("debug::raw final rewards from loop:", final_rewards[:20], final_rewards.mean())
                            # print("reward_tensor example (first 5 rows, first 10 tokens each):")
                            # for i in range(min(5, reward_tensor.shape[0])):
                            #     print(f"[{i}] {reward_tensor[i][:10].tolist()}")
                            ...
                            # for i in range(min(5, reward_tensor.shape[0])):
                            #     print(f"[{i}] {reward_tensor[i][:10].tolist()}")
                            # reward_tensor = final_rewards.unsqueeze(1).repeat(1, reward_tensor.shape[1])
                            # pad for log_prob
