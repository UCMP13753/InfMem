...
class ChatCompletionProxy(ChatCompletionScheduler):
    ...
    # async def _completions_aiohttp(self, address: str, **complete_request) -> ChatCompletion:
    ...
    # async def get_completions_text(
    #     self,
    #     model=None,
    #     eos_token_text=None,
    #     **complete_request,
    ...
    #     Args:
    #         **complete_request: dict, request parameters same as OpenAI AsyncCompletions.create.
    #             OpenAI API reference: https://platform.openai.com/docs/api-reference/chat/create
    #     """
    #     model = model or self.model_name
    #     if "extra_headers" not in complete_request:
    #         complete_request["extra_headers"] = {}
    #     if "prompt" not in complete_request:
    #         complete_request["prompt"] = ""
    #     input_prompt = complete_request.get("prompt","")
    ...
    #     try:
    #         # TODO: OpenAI client uses httpx, seems to have performance issue in high concurrency requests.
    #         completions = await self._completions_aiohttp(address, model=model, **complete_request)
    #         print(completions["choices"][0])
    #         generation = completions["choices"][0]["message"]
    #         print(generation)
    #         if eos_token_text and (eos_token_text not in generation):
    #             if "</think>" not in generation:
    #                 # ---------- insert closing tag ----------
    #                 generation = generation.rstrip() + "\n\nConsidering the limited time, I must stop thinking now.\n</think>\n\n"
    #             followup_prompt = input_prompt + generation
    #             complete_request["prompt"] = followup_prompt
    #             try:
    #                 completions_2 = await self._completions_aiohttp(address, model=model, **complete_request)
    #             except Exception as e:
    #                 # Let user handle the exception
    #                 exception = e
                
    #             answer_part = completions_2["choices"][0]["message"].strip()
    #             generation = generation + answer_part
    #             setattr(completions.choices[0], "message", generation)
    #     except Exception as e:
    #         # Let user handle the exception
    #         exception = e
    #         print("exception:",exception)
