...
class MemoryAgent(RAgent):
    def __init__(self, tokenizer:PreTrainedTokenizer, config: MemoryConfig):
        ...
        self.end_think  = tokenizer.encode("</think>", add_special_tokens=False)   # [522, 1708, 29] special token for qwen2.5
    ...
    def start(self, gen_batch: DataProto, timing_raw: dict):
        ...
        self.rewards = torch.zeros(self.bsz, dtype=torch.float32)
        self.rewards_time = torch.zeros(self.bsz, dtype=torch.int32)
    ...
    def update(self, gen_output: DataProto, not_that_correct = 0.5) -> DataProto:
        ...
        def normalize_text(s: str) -> str:
            # basic cleanup: strip, collapse whitespace, lower if you want
            s = s.strip()
            s = re.sub(r"\s+", " ", s)
            return s

        def contains_answer_text(mem_ids, a, tokenizer) -> bool:
            # mem_ids: 1D tensor or list[int]
            ...
            raw_sentences = mem_text.split(".")
            sentences = [normalize_text(s).lower() for s in raw_sentences if normalize_text(s)]
            ...
            # ---- Find which sentence contains gt_norm ----
            for idx, sent in enumerate(sentences):
                if gt_norm and gt_norm in sent:
                    # idx 从 0 开始，需要 +1
                    reward = (idx + 1) / total
                    return True, reward
        ...
        if not self.is_final:
            ...
            for local_i, global_i in enumerate(act_idx):
                ...
                for a in ans:
                    ok, reward = contains_answer_text(
                        mem_ids,
                        a,
                        self.tokenizer,
                    )
                ...
                if found and self.rewards_time[global_i] >= 0:
                    # haven't loss
                    self.rewards[global_i] += reward
                    self.rewards_time[global_i] += 1
                elif self.rewards_time[global_i] > 0:
                    # if used to found, locked in not_that_correct
                    self.rewards[global_i] = 0.1
                    self.rewards_time[global_i] = -1
                    # else, haven't appeared yet, keep as 0.0
    ...
    def end(self):
        ...
        del self.memory
        del self.messages
        final_rewards = self.rewards.clone()

        mask = (self.rewards_time > 0) & (self.rewards > 0)
        final_rewards[mask] = self.rewards[mask] / self.rewards_time[mask].float()
        ...
        return final_mask, sample_index, None
    ...
    def log_step(self, gen_output):
        ...
        if self.active_mask[0]:
            ...
            if  self.rewards_time[0] > 0:
                current_reward = self.rewards[0]/self.rewards_time[0]
            else:
                current_reward = self.rewards[0]
            logger.info(f"[CORRECT ANSWER] {len(self.answers_ids[0])} answers in total, first is {correct_answer}, current reward is {current_reward}")
