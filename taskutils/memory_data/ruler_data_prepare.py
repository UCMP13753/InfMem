...
mode = "train" # eval
save_name = f"{mode}_{args.task}_{args.max_seq_length}.json" # make sure to seperate 
...
def main():
    ...
    # Gather
    import glob
    # files = glob.glob(str(temp_dir / f"sft_training_{args.task}_{args.max_seq_length}.json.*"))
    files = glob.glob(str(temp_dir / f"{save_name}.*"))
