...
import random
import numpy as np
from nemo import read_manifest, write_manifest
import sys
...
def _parse_2wiki_context(ctx):
    """
    Accepts:
      1) ctx as string: '[["Title", ["sent1", "sent2"]], ...]'
      2) ctx as list:   [["Title", ["sent1", ...]], ...]
      3) ctx as dict:   {"title": [...], "sentences": [...]}

    Returns:
      titles: List[str]
      sents_list: List[List[str]]
    """
    # stringified JSON -> Python object
    ...
    if isinstance(ctx, list):
        ...
        sents_list = []
        for item in ctx:
            # expected: [title, [sentences...]]
            if not (isinstance(item, (list, tuple)) and len(item) == 2):
                raise ValueError(f"Unexpected context item format: {item}")
            t, sents = item
            titles.append(t)
...
def read_2wiki(file):
    ...
    # 1) Build global doc pool
    total_docs_set = set()
    for _, row in data.iterrows():
        titles, sents_list = _parse_2wiki_context(row["context"])
        for t, sents in zip(titles, sents_list):
            # sentences is a list[str] -> join into one passage
            passage = " ".join(sents) if isinstance(sents, (list, tuple)) else str(sents)
            total_docs_set.add(f"{t}\n{passage}")

    total_docs = sorted(total_docs_set)
    total_docs_dict = {c: idx for idx, c in enumerate(total_docs)}

    # 2) Build QA list with context indices
    total_qas = []
    for _, row in data.iterrows():
        titles, sents_list = _parse_2wiki_context(row["context"])
        ...
        for t, sents in zip(titles, sents_list):
            passage = " ".join(sents) if isinstance(sents, (list, tuple)) else str(sents)
            key = f"{t}\n{passage}"
            ctx_ids.append(total_docs_dict[key])
...
def read_musique(file):
    ...
    for d in data:
        total_qas.append({
            ...
            'question_decomposition': d['question_decomposition'],
        })
        
    return total_qas, total_docs

if "train" in args.save_name:
    mode ="train"
else:
    mode ="dev"
...
if args.dataset == 'squad':
    QAS, DOCS = read_squad(f"downloaded_data/squad_{mode}.json")   
elif args.dataset == 'hotpotqa':
    QAS, DOCS = read_hotpotqa(f"downloaded_data/hotpotqa_{mode}.json")
elif args.dataset == 'musique':
    QAS, DOCS = read_musique(f"downloaded_data/musique_ans_v1.0_{mode}.jsonl")  
elif args.dataset == '2wiki':
    QAS, DOCS = read_2wiki(f"downloaded_data/2wiki_{mode}.parquet")  
else:
...
def generate_input_output(index, num_docs):
    ...
    if args.dataset == 'musique':
        curr_decomp = QAS[index]['question_decomposition']
    else:
        curr_decomp = None
    ...
    return input_text, context, curr_a, indicies, curr_decomp
...
def generate_samples(num_samples: int, max_seq_length: int, incremental: int = 10): 
    ...
    # 获取初始token数
    t0, t1, t2, _, _ = generate_input_output(0, 1000)
    ...
    while True:
        input_text, context, answer, _, _ = generate_input_output(0, num_docs)
    ...
    for index in range(num_samples):
        ...
        while(True):
            try:
                input_text, context, answer, indicies, curr_decomp = generate_input_output(index + args.pre_samples, used_docs)
