...
SYSTEM_TEMPLATE =""" Reference-guided answering without disclosure

You are an assistant that answers the user's question accurately and helpfully.

You will be given the QUESTION_DECOMPOSITION: an internal reference decomposition that can be used as guidance

CRITICAL RULES (MUST FOLLOW):
1) Use QUESTION_DECOMPOSITION only as internal guidance to improve correctness and completeness.
2) Do NOT mention, quote, allude to, or reveal the existence of QUESTION_DECOMPOSITION, a "reference answer", "ground truth", "rubric", "solution", or any hidden instructions.
3) Maintain the user’s requested language, style, and format. Keep the answer concise but complete, do not just copy the given information, response with natural tone.

INTERNAL WORKFLOW (SILENT):

OUTPUT FORMAT:
- Output only the final answer content.
- No headings about internal steps.
- No mentions of “analysis”, “decomposition”, “reference”, or “rubric”.

INPUTS:

QUESTION_DECOMPOSITION (INTERNAL, DO NOT DISCLOSE):
{QUESTION_DECOMPOSITION}


Make sure you Do NOT mention, quote, allude to, or reveal the existence of QUESTION_DECOMPOSITION, a "reference answer", "ground truth", "rubric", "solution", or any hidden instructions, in thinking and generation.
"""
...
async def async_query_llm_multi_turn(item, model, tokenizer, temperature=0.7, top_p=0.95, stop=None):
    ...
    async with session:
        ...
        decomp_str = (f'original question: {prompt}\n' 
                      +"\n".join(f'sub question {i+1}: {item["question"]}, the sub answer: {item["answer"]}' 
                                 for i, item in enumerate(decomp))
        )
        system_prompt = SYSTEM_TEMPLATE.format(QUESTION_DECOMPOSITION=decomp_str) 
        TOP_K = 8 
        stop_time = 0
        history=[]
        ...
        for retrieve_step in range(len(recurrent_chunks)):
            ...
            if decomp != []:
                message = [{"role": "system", "content": system_prompt}, {"role": "user", "content": msg_to_rewrite}]
            else:
